function [errmsgs warnmsgs] = unit_class_ridge(varargin)

% USAGE :[ERRMSGS WARNMSGS] = TEST_(type)
% 
% This is a script that tests the functionality of the ridge
% regression scripts, train_ridge.m and test_ridge.m  It tests that
...
% train_ridge
try
  train_ridge(rand(5,5), rand(1,5), [], [])
  err('train_ridge should require penalty parameter');
end
try
  class_args.penalty = 0;
  train_ridge(rand(5,5), rand(5,1), class_args);
  err('non-row vector targets should not be allowed');
end
try
  class_args.penalty = 0;
  train_ridge(rand(5,5), rand(1,3), class_args);
  err('should detect differing columns of targets and pattern');
end
try
  class_args.penalty = 0;
  train_ridge(NaN(5,5), rand(1,5), class_args);
  err('should detect NaN data');
end
try
  class_args.penalty = 0;
  train_ridge(rand(5,5), NaN(1,5), class_args);
  err('should detect NaN data');
end
...
try
  class_args.penalty = 0;
  test_ridge(NaN(5,5), rand(1,5), class_args);
  err('should detect NaN data');
end
try
  class_args.penalty = 0;
  test_ridge(rand(5,5), NaN(1,5), class_args);
  err('should detect NaN data');
end
try
  test_ridge(rand(5,5), rand(1,5));
  err('should require scratchpad from train_ridge');
end
try
  scratchpad.ridge.betas = rand(5,1);
  test_ridge(rand(5,5), rand(5,1), scratchpad);
...
try  
  ...
  scratchpad = train_ridge(X, Y, class_args);
  ...
  [acts scratchpad] = test_ridge(X, Y, scratchpad);
  d = sum(abs(Y - acts));
  if d > 1e-12
...
try
  ...
  scratchpad = train_ridge(X, scramY, class_args);
  [acts scratchpad] = test_ridge(X, scramY, scratchpad);
