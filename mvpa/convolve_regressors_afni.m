...
%
% Takes a set of regressors and convolves each condition with a
% gamma haemodynamic response function using AFNI's 'waver' command
% - separately for each run
%
% Adds the following objects:
% - a new regressors object called NEW_REGSNAME
%
% The AFNI waver function requires a .1d text file as input containing
% a column of numbers for a given condition which it will convolve
% with the HRF. It then creates a new .1d file with the convolved
% numbers. This function will create a set of temporary .1d files, one
% for each condition, called NEW_REGSNAME_<cond#>.1d. It will then
% call waver to create a corresponding bunch of
% NEW_REGSNAME_conv_<cond#>.1d files, load them in to matlab,
% concatenate them and create a new regressors object called
% NEW_REGSNAME to keep them in. It will then delete all the temporary
% 1d files it created, unless KEEP_FILES is true.
%
% Note: this takes a few seconds, because the waver command seems
% pretty slow for what it does
%
% NEW_REGSNAME (optional, default = OLD_REGSNAME + 'conv')
%
% OVERWRITE_IF_EXIST (optional, default = false). By default, this
% function will halt if any of the input or output .1d files that
% it tries to create already exist. Set this to true if you want it
% to overwrite existing files
%
% TR_SIZE_IN_SECONDS (optional, default = 2). This is the number of
% seconds that each TR lasts. AFNI needs to know this
%
% SCALE_TO_ONE (optional, default = true). When you convolve your
% regressors, the non-zero values in them get much larger
% (depending on how many timepoints you have). Sometimes, you may
% have problems if your regressor values go above 1, so this will
% scale the convolved regressors so that the maximum value in them
% is 1. Set to false if you don't want to do any scaling
%
...
%
% WAVER_LOCATION (optional, default = ''). If empty, this assumes
% that the 'waver' command is somewhere in your path, and just
% calls it directly. However, if waver isn't in your path, this
% will prepend the WAVER_LOCATION pathname to the waver call,
% e.g. '/usr/bin/afni/'
%
...
wavexec = sprintf('%s -GAM -dt %i -numout %i -input %s > %s', ...
		  fullfile(args.waver_location,'waver'), ...
