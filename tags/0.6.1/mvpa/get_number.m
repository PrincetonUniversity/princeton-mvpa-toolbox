...
found_already = false;
for p=1:nbr_objects
  if ~isfield(objcell{p},'name')
    error( sprintf('This %s object (%i) doesn''t have a name',objtype,p) );
  end
...
if ~objno && ~args.ignore_absence
  ...
  if ~isempty(type_matches_str)
    disp( sprintf('Did you mean one of these objtypes?%s',type_matches_str) );
  end
