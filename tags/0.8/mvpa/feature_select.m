...
% - mask group based on the statmaps called sprintf('%s%i',NEW_MASKSTEM,THRESH)
%
% DATA_PATIN should be the name of the pattern object that contains
% voxel (or other feature) values that you want to create a mask
% of. If DATA_PATIN is a group_name, then this will use a different
% member of the group for each iteration.
%
...
%
% For each iteration: call the ANOVA on the DATA_PATIN data, which
...
% STRIPPED_NAME). The name of the new statmap pattern group to be
% created. By default, this will be 'anova' if STATMAP_FUNCT =
% 'statmap_anova' etc.
%
...
% gets created from the ANOVA statmap. If THRESH = [], the
% thresholding doesn't get run
%
...
args.new_maskstem = sprintf('%s%s',args.new_maskstem,num2str(args.thresh));
