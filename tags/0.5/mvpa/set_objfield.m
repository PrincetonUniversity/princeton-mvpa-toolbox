...
% If you're setting a field that doesn't exist, warn the user, to
% highlight possible typos
% But in case they're deliberately setting the field, check IGNORE_ABSENCE
if ~isfield(obj,fieldname) & ~args.ignore_absence
...
if isempty(newval) & ~args.ignore_empty & ~isempty(obj.(fieldname))
  warning('About to overwrite a field with an empty one');
end
