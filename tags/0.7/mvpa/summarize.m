...
% PROPVAL first pass. Must know objtype to calculate display_groups default
defaults.objtype = 'subj';
if nargin == 2
  if get_tot_subj_size(subj,'subj') < 35
    defaults.display_groups = 1;
  else
    defaults.display_groups = -1;
  end
  ...
  defaults.display_groups = 'temporary variable to be overwritten in propval stage deux';
  args = propval(varargin,defaults);
  if get_tot_subj_size(subj,args.objtype) < 35
    defaults.display_groups = 1;
  else
    defaults.display_groups = -1;
  end
