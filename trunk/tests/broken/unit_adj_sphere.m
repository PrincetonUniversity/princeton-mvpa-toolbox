function [errmsgs warnmsgs] = unit_adj_sphere(varargin)

% Unit tests for ADJ_SPHERE.M
%
% [ERRMSGS WARNMSGS] = UNIT_ADJ_SPHERE(...)
%
...
mask = zeros(5,5,5);
mask(100) = 1;
adj_list = adj_sphere(mask);
if length(adj_list)~=1
  errmsgs{end+1} = 'Scenario 1:Should only have one cell in adj_list';
  return
end

if ~isempty(adj_list{1})
  errmsgs{end+1} = 'Scenario 1: single voxel adj should be empty';
end
...
adj_list = adj_sphere(mask,sphere_args);
...
if ~isequal(adj_list{p464idx},p454idx)
...
if ~isequal(test_cv_cur_adjs,real_cv_cur_adjs)
...
adj_list = adj_sphere(ones([10 10 10]),sphere_args);

is_empty_adj_list = check_empty_adj_list(adj_list);

if ~is_empty_adj_list
  errmsgs{end+1} = 'Zero radius produces non-empty adj cells';
end
...
[real_spheres_p_idx scratch real_spheres_mask_idx] = ...
    adj_sphere(mask,sphere_args);
exclude_center = scratch.args.cuboid_args.args.exclude_center;
...
for m=1:nTotalVox
  ...
  if mask(m)
    ...
    % spheres created by ADJ_SPHERE and check that they
    % match up
